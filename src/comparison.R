...
get_histogram <- function(data, variable, variable_label, method, method_label, binwidth, x_range, y_range){
  histogram <- ggplot(data = data, aes(x = !!sym(variable))) +
...
get_distribution <- function(data, variable){
  data %>%
    summarize(
      ...
      `quantile 2.5%` = get_quantile(!!sym(variable), probs = c(0.025)),
      `quantile 25%` = get_quantile(!!sym(variable), probs = c(0.25)),
      `quantile 75%` = get_quantile(!!sym(variable), probs = c(0.75)),
      `quantile 97.5%` = get_quantile(!!sym(variable), probs = c(0.975)),
