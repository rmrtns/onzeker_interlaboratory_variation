...
results_filt <- do.call(rbind, lapply(data_list_results, function(df) {
...
reference_unique <- reference_filt %>%
  group_by(anl, ctm) %>%
  slice(which.min(match(Methode, c("Referentiewaarde", "Expertwaarde")))) %>%
  ungroup() %>% 
  rename(ConsensusMethode = Methode, ConsensusWaarde = Gemiddelde)

skml_merged <- left_join(reference_unique, results_filt, 
