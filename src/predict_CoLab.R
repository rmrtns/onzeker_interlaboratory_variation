...
get_lp_prediction_CoLab <- function(X.unscaled){
  ...
  if (!all(coef.names.no.int %in% names(X))) {
  ...
  X.ordered <- subset(X, select = coef.names.no.int)
  lp <- rowSums(mapply('*', X.ordered, coefs[coef.names.no.int]))
  lp + coefs["(Intercept)"]
}

get_ordinal_prediction_CoLab <- function(X.unscaled){
  lps <- get_lp_prediction_CoLab
  cutoffs <- c( -1.64, -2.34, -3.29, -4.03, -5.83)
  cut(lps, c(Inf, cutoffs, -Inf), labels = seq(0, length(cutoffs)), 
      ordered_result = T)
}

get_binary_prediction_CoLab <- function(X.unscaled){
  lps <- get_lp_prediction_CoLab
  lps >= -1.64
}